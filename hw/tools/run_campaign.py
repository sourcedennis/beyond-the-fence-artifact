...
def main():
    ...
    stress_params = OUTPUT_DIR / f"{args.machine}-{args.mode}-params.txt"
    write_stress_params(stress_params, args.iterations)
    log_path = OUTPUT_DIR / f"{args.machine}-{args.mode}.txt"

    with open(log_path, "w") as output:
        output.write(f"Mode: {args.mode}\n")
        output.write(f"Machine: {args.machine}\n")
        output.write(f"CUDA arch: {arch}\n")
        output.write(f"Iterations per runner: {args.iterations}\n")
        for test in tests:
